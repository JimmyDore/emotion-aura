 ...
 *
 * Uses finger-curl detection (TIP vs PIP y-position) and thumb-index
 * pinch distance to classify hand pose into one of four gesture types.
 *
 * Priority order: pinch > fist (attract) > open hand (push) > none
 *
...
export function classifyGesture(landmarks: NormalizedLandmark[]): GestureType {
  ...
  // A curled finger has TIP below (greater y) its PIP joint.
